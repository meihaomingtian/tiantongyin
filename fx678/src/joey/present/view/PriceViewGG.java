...
public class PriceViewGG extends Activity {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		if (ev.getAction() == MotionEvent.ACTION_MOVE) {
			// priceNameListView.dispatchTouchEvent(ev);
			// priceListView.dispatchTouchEvent(ev);
		}
	...
	private void updateData(String type) {
		// listData =
		// util.getPriceData("http://mhth.fx678.com/quotelist.aspx?key="
		// + type, type);
		listData = util.getPriceData("http://mhth.fx678.com/quotelist.aspx?key=" + type, type);
	...
	public void showNew(PriceData new_timeNow) {
		...
		for (int i = 0; i < mData.size(); i++) {
			...
			if (code.equals(timeNow.getPrice_code())) {
				...
				map.put(Const.PRICE_LASTCLOSE, String.valueOf(df_base.format(Float.parseFloat(timeNow.getPrice_lastclose()))));
				// map.put(Const.PRICE_LASTSETTLE,
				// String.valueOf(df_base.format(Float.parseFloat(timeNow.getPrice_lastsettle()))));
				...
				// map.put(Const.PRICE_AVERAGE,
				// String.valueOf(df_base.format(Float.parseFloat(timeNow.getPrice_average()))));
	...
	public void showData() {
		...
		// LinearLayout.LayoutParams ff = new
		// LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT,
		// 45 * listData.size());
		// priceNameListView.setLayoutParams(ff);
		// LinearLayout.LayoutParams ff2 = new
		// LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT,
		// 45 * listData.size());
			...
			while (running) {
				...
				try {
					...
					if (str.indexOf("{") >= 0 && str.indexOf("}") >= 0) {
						...
						// log.i("temp", "code->" + temp.getPrice_code());
						if (temp != null) {
	...
	private class SendActiveThread extends Thread {
		...
		public void run() {
			while (running) {
				...
				if (refreshListData != null && refreshListData.size() > 50) {
					refreshListData.clear();
				}
			}
		}
